...
export function decodeCursor(cursor) {
    const value = decodeOpaqueCursorObject(cursor, "Invalid cursor: expected an opaque cursor returned by mysterium_read_book.", "Invalid cursor: cursor payload must be an object.");
    if (value.version !== 1)
        throw new Error("Invalid cursor: unsupported cursor version; restart without a cursor.");
    if (typeof value.bookSlug !== "string")
        throw new Error("Invalid cursor: missing book binding.");
    if (typeof value.chapterSlug !== "string")
        throw new Error("Invalid cursor: missing chapter binding.");
    if (value.section !== null && typeof value.section !== "string")
        throw new Error("Invalid cursor: invalid section binding.");
    if (!Number.isInteger(value.maxChars) || value.maxChars < 1 || value.maxChars > SERVER_MAX_CHARS) {
        throw new Error("Invalid cursor: invalid character limit.");
    }
    if (!Number.isInteger(value.blockIndex) || value.blockIndex < 0) {
...
export function validateReadBookRequest(request) {
    assertSlug(request.bookSlug, "book_slug");
    if (request.chapterSlug !== undefined)
        assertSlug(request.chapterSlug, "chapter_slug");
    ...
    if (cursorPayload) {
        if (mode !== "content")
            throw new Error("A cursor can only continue a content request.");
        if (cursorPayload.bookSlug !== request.bookSlug ||
...
export function paginateBlocks(blocks, maxChars, start = { blockIndex: 0, offset: 0 }) {
    const page = paginateSegments(blocks.map((block, index) => ({
        text: `${block.text}${index < blocks.length - 1 ? "\n\n" : ""}`,
        values: block.imageIds,
    })), maxChars, SERVER_MAX_CHARS, { segmentIndex: start.blockIndex, offset: start.offset }, {
...
function selectSection(extracted, selector) {
    if (!selector)
        return { blocks: extracted.blocks };
    let matches = extracted.outline.filter((entry) => entry.id === selector);
    if (matches.length === 0)
        matches = extracted.outline.filter((entry) => entry.title === selector);
    if (matches.length === 0)
        throw new Error(`Section '${selector}' was not found in the chapter outline.`);
    if (matches.length > 1) {
    ...
    if (start < 0)
        throw new Error(`Section '${selector}' has no readable content.`);
...
async function extractBookPage(context, request) {
    ...
    const extracted = await page.evaluate(({ bookSlug, isBookOutline }) => {
        ...
        const normalize = (value) => (value ?? "").replace(/\s+/g, " ").trim();
        const slugify = (value) => normalize(value).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "section";
        const uniqueId = (prefix, entryTitle, counts) => {
            const base = `${prefix}-${slugify(entryTitle)}`;
            const count = (counts.get(base) ?? 0) + 1;
            counts.set(base, count);
            return `${base}-${count}`;
        };
        const parentFor = (level, stack) => {
        ...
        if (isBookOutline) {
            ...
            const seen = new Set();
            const stack = [];
            const outlineRoot = root.querySelector("[data-testid*='toc' i], .toc, .compendium-toc, [class*='TableOfContents'], [aria-label*='contents' i]") ?? root;
            outlineRoot.querySelectorAll("a[href]").forEach((node) => {
                ...
                if (!chapterSlug || seen.has(identity))
                    return;
                seen.add(identity);
        ...
        const imageIdsFor = (element) => Array.from(element.querySelectorAll("img"))
            .map((image) => registerImage(image)?.id)
        ...
        const renderInline = (node) => {
            ...
            if (["script", "style", "button", "svg"].includes(tag))
                return "";
            if (tag === "img") {
                const image = registerImage(element);
        ...
        const renderList = (list, depth = 0) => {
            ...
            Array.from(list.children).filter((child) => child.tagName.toLowerCase() === "li").forEach((item, index) => {
                const direct = Array.from(item.childNodes).filter((node) => !(node.nodeType === Node.ELEMENT_NODE && ["ul", "ol"].includes(node.tagName.toLowerCase())));
        ...
        const renderTable = (table) => {
            const rows = Array.from(table.querySelectorAll("tr")).map((row) => Array.from(row.querySelectorAll(":scope > th, :scope > td")).map((cell) => normalize(renderInline(cell)).replace(/\|/g, "\\|"))).filter((row) => row.length > 0);
            if (rows.length === 0)
                return "";
        ...
        const candidates = Array.from(root.querySelectorAll(blockSelector)).filter((element) => {
            ...
            while (parent && parent !== root) {
                if (parent.matches("p, ul, ol, blockquote, table, pre, figure"))
                    return false;
        ...
        candidates.forEach((element) => {
            ...
            let text = "";
            if (heading)
                text = `${"#".repeat(heading.level)} ${heading.title}`;
            else if (tag === "p")
                text = normalize(renderInline(element));
            else if (tag === "ul" || tag === "ol")
                text = renderList(element);
            else if (tag === "blockquote")
                text = normalize(renderInline(element)).split("\n").map((line) => `> ${line}`).join("\n");
            else if (tag === "table")
                text = renderTable(element);
            else if (tag === "pre")
                ...
                const metadata = image ? registerImage(image) : null;
                text = metadata ? `[Image ${metadata.id}${metadata.alt ? `: ${metadata.alt}` : ""}]${metadata.caption ? `\n\n${metadata.caption}` : ""}` : "";
            }
            else if (tag === "img") {
                const metadata = registerImage(element);
                text = metadata ? `[Image ${metadata.id}${metadata.alt ? `: ${metadata.alt}` : ""}]` : "";
            }
            text = text.trim();
            if (!text)
                return;
            const directImage = tag === "img" ? registerImage(element) : null;
            blocks.push({
                text,
...
export async function readBook(context, input) {
    ...
    const chunk = paginateBlocks(selected.blocks, request.maxChars, cursorPayload ? { blockIndex: cursorPayload.blockIndex, offset: cursorPayload.offset } : undefined);

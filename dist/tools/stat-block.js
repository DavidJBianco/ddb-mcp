...
export function normalizeCreatureName(value) {
    return value
        .normalize("NFKC")
        .replace(/[‘’]/g, "'")
        .replace(/[‐‑‒–—]/g, "-")
...
export function validateStatBlockRequest(request) {
...
export function creatureIdFromUrl(value) {
    try {
...
function candidateFromSearch(result) {
...
export function selectStatBlockCandidates(query, results, legacy = "include") {
    ...
    if (legacy === "exclude")
        return { eligible: allExact.filter((candidate) => !candidate.legacy), allExact };
    if (legacy === "only")
        return { eligible: allExact.filter((candidate) => candidate.legacy), allExact };
...
function safeMonsterUrl(value, expectedId) {
    let url;
    try {
...
function inaccessibleMessage(page) {
    try {
...
async function extractStatBlockFromPage(page, creatureId, hintedUrl) {
    if (!CREATURE_ID_PATTERN.test(creatureId))
        throw new Error("creature_id must contain only digits.");
    ...
    try {
        ...
        try {
            ...
            if (!isDdbHostname(redirected.hostname))
                throw new StatBlockInaccessibleError(inaccessibleMessage(page));
        }
        catch (urlError) {
            if (urlError instanceof StatBlockInaccessibleError)
                throw urlError;
        }
        throw error;
    }
    ...
    const extracted = await page.evaluate(() => {
        ...
        const hasHitPoints = (value) => /(?:\bHit Points\b|\bHP\s*\d)/i.test(value);
        ...
        const liveRoot = document.querySelector("[data-testid*='stat-block' i]") ??
        ...
        if (!liveRoot)
            return { extractionError: "unrecognized_layout" };
        ...
        if (!hasArmorClass(bodyText) || !hasHitPoints(bodyText))
            return { extractionError: "incomplete_stat_block" };
        ...
        const meta = normalize(root.querySelector("[class*='meta' i], [class*='type' i]")?.textContent ??
        ...
        root.querySelectorAll(`${semanticAttributeSelector}, p, li`).forEach((node) => {
            const containsNestedAttributeRow = node.matches(semanticAttributeSelector) &&
                Array.from(node.querySelectorAll(semanticAttributeSelector)).some((descendant) => Boolean(descendant.querySelector(":scope > strong, :scope > b, [class*='label' i]")));
            if (containsNestedAttributeRow)
                return;
        ...
        const sectionKind = (title) => title.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "section";
        ...
        for (const sectionHeading of headings) {
            ...
            if (!title || title === name || /Description|Comments|Lore|Habitat/i.test(title))
                continue;
            const entries = [];
            const seenEntryNodes = new Set();
            const addEntry = (node) => {
                if (seenEntryNodes.has(node))
                    return;
                seenEntryNodes.add(node);
                const text = normalize(node.textContent);
                if (!text || /^(?:Description|Comments)$/i.test(text))
                    return;
            ...
            if (entries.length > 0)
                sections.push({ title, kind: sectionKind(title), entries });
        }
        ...
        if (meta)
            markdownBlocks.push(`_${meta}_`);
        if (legacy)
            markdownBlocks.push("**Legacy**");
        for (const attribute of attributes)
            markdownBlocks.push(`**${attribute.label}:** ${attribute.value}`);
    ...
    if (!extracted)
        throw new StatBlockInaccessibleError();
    if ("extractionError" in extracted) {
        if (extracted.extractionError === "inaccessible")
            throw new StatBlockInaccessibleError();
...
async function withStatBlockPage(context, operation) {
...
export async function extractStatBlock(context, creatureId, hintedUrl) {
    return withStatBlockPage(context, (page) => extractStatBlockFromPage(page, creatureId, hintedUrl));
}
export async function getStatBlock(context, input) {
    return withStatBlockPage(context, async (page) => {
        ...
        try {
            ...
            if (!(error instanceof StatBlockInaccessibleError) || resolution.query === null || resolution.normalizedQuery === null)
                throw error;
            ...
            const ordinary = envelope.results.filter((result) => result.type !== "sourcebook");
...
async function resolveStatBlockOnPage(context, page, input) {
    const request = validateStatBlockRequest(input);
    ...
    if (eligible.length === 0)
        return { kind: "not_found", query, normalizedQuery, legacy: request.legacy, candidates: [] };
    if (eligible.length > 1)
        return { kind: "candidates", query, normalizedQuery, legacy: request.legacy, candidates: eligible };
    return { kind: "resolved", query, normalizedQuery, legacy: request.legacy, candidate: eligible[0] };
}
export async function resolveStatBlock(context, input) {

...
import { codePointLength, decodeOpaqueCursorObject, encodeOpaqueCursor, paginateSegments, splitTextAtNewlines, } from "./pagination.js";
...
function assertMaxChars(maxChars) {
...
export function encodePageCursor(payload) {
    return encodeOpaqueCursor(payload);
}
export function decodePageCursor(cursor) {
    const value = decodeOpaqueCursorObject(cursor, "Invalid page cursor: expected an opaque cursor returned by mysterium_read_page.", "Invalid page cursor: cursor payload must be an object.");
    if (value.version !== 1)
        throw new Error("Invalid page cursor: unsupported cursor version; restart without a cursor.");
    if (typeof value.url !== "string" || !isAllowedDdbUrl(value.url))
        throw new Error("Invalid page cursor: invalid page binding.");
    if (!Number.isInteger(value.maxChars))
        throw new Error("Invalid page cursor: invalid character limit.");
    assertMaxChars(value.maxChars);
    if (!Number.isInteger(value.segmentIndex) || value.segmentIndex < 0 ||
...
export function validatePageContentRequest(request) {
...
export function validatePageScreenshotRequest(request) {
    const scope = request.scope ?? "viewport";
    const selector = request.selector?.trim();
    if (scope === "viewport" && selector !== undefined)
        throw new Error("selector is only valid when screenshot scope is 'element'.");
    if (scope === "element" && !selector)
        throw new Error("selector is required when screenshot scope is 'element'.");
...
export function paginatePageText(text, maxChars, start = { segmentIndex: 0, offset: 0 }) {
...
async function extractPage(page) {
    const extracted = await page.evaluate(() => {
        ...
        clone.querySelectorAll("script, style, nav, footer, noscript, template, .ad-container, .advertisement, [hidden], [aria-hidden='true']").forEach((element) => element.remove());
...
async function assertReadablePage(page) {
    if (!isAllowedDdbUrl(page.url()))
        throw new Error("The current page is outside D&D Beyond and cannot be read.");
    if (!(await isLoggedInOnCurrentPage(page)))
        throw new AuthenticationRequiredError();
}
function pageEnvelope(operation, requestedUrl, extracted, request) {
    const cursorPayload = request.cursor ? decodePageCursor(request.cursor) : undefined;
    const contentFingerprint = fingerprint(extracted.text);
    if (cursorPayload) {
        if (cursorPayload.url !== extracted.url)
            throw new Error("Page cursor does not match the current page URL; restart without a cursor.");
...
export async function readPage(context, input = {}) {
    ...
    await assertReadablePage(page);
    return pageEnvelope(request.url === undefined ? "current_page" : "navigate", request.url ?? null, await extractPage(page), request);
}
...
function assertScreenshotBounds(width, height) {
    if (!Number.isFinite(width) || !Number.isFinite(height) || width < 1 || height < 1 ||
        width > MAX_SCREENSHOT_DIMENSION || height > MAX_SCREENSHOT_DIMENSION || width * height > MAX_SCREENSHOT_PIXELS) {
        throw new Error(`Screenshot dimensions must not exceed ${MAX_SCREENSHOT_DIMENSION}x${MAX_SCREENSHOT_DIMENSION} or ${MAX_SCREENSHOT_PIXELS} total pixels.`);
    }
}
export async function capturePageScreenshot(context, request = {}) {
    ...
    if (validated.scope === "element") {
        ...
        if (count !== 1)
            throw new Error(`Element screenshot selector must match exactly one element; matched ${count}.`);
        if (!(await locator.isVisible()))
            throw new Error("Element screenshot target is not visible.");
        const box = await locator.boundingBox();
        if (!box)
            throw new Error("Element screenshot target has no visible bounding box.");

...
import { characterDetailSchema, characterListEnvelopeSchema, characterPortraitMetadataSchema, campaignDetailEnvelopeSchema, campaignListEnvelopeSchema, libraryEnvelopeSchema, readBookResultSchema, searchEnvelopeSchema, statBlockResolutionSchema, statBlockResultSchema, statBlockSchema, } from "./tool-contracts.js";
...
function statBlockRequest(query, creatureId, legacy) {
...
export function createServer(getContextForTool = getSharedContext, options = {}) {
    ...
    registerAppTool(server, "mysterium_view_stat_block", {
    ...
    registerAppTool(server, "read_stat_block_for_app", {
    ...
    registerAppResource(server, STAT_BLOCK_RESOURCE_URI, STAT_BLOCK_RESOURCE_URI, { mimeType: RESOURCE_MIME_TYPE }, async () => ({
        contents: [{
    ...
    server.registerTool("mysterium_get_campaign", {
    ...
    // ─── mysterium_navigate ─────────────────────────────────────────────────────────────
    server.tool("mysterium_navigate", "Navigate to any D&D Beyond URL and return the page's text content. Only dndbeyond.com URLs are allowed.", {
        url: z
            .string()
    ...
    // ─── mysterium_interact ─────────────────────────────────────────────────────────────
    server.tool("mysterium_interact", "Interact with the currently loaded D&D Beyond page by clicking, filling a form field, or taking a screenshot.", {
        action: z
    ...
    server.tool("mysterium_current_page", "Return the text content of the currently loaded page in the browser.", {}, async () => {
        try {
            const context = await getContextForTool();

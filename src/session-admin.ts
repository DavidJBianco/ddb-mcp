...
function emit(result: AdminResult): void {
...
async function readStandardInput(): Promise<Buffer> {
  ...
  for await (const chunk of process.stdin) {
    ...
    size += bytes.length;
    if (size > MAX_SESSION_BYTES) throw new Error("Candidate session state exceeds the size limit.");
    chunks.push(bytes);
  }
  return Buffer.concat(chunks);
}

async function verifyLive(path: string): Promise<void> {
...
async function importSession(live: boolean): Promise<void> {
  ...
  try {
    ...
    if (live) await verifyLive(candidatePath);
    await writeFile(
      metadataCandidate,
      `${JSON.stringify({ schemaVersion: SESSION_SCHEMA_VERSION, helperVersion, serverVersion: PACKAGE_VERSION, createdAt }, null, 2)}\n`,
      { mode: 0o600, flag: "wx" }
    );
...
async function validateSession(live: boolean): Promise<void> {
...
export async function main(args = process.argv.slice(2)): Promise<void> {
  ...
  if (options.some((option) => option !== "--live")) throw new Error("Unsupported session-admin option.");
  ...
  switch (command) {
    ...
    case "validate":
      ...
      return;
    case "reset":
      if (live) throw new Error("reset does not accept --live.");
      await resetSession();
      return;
    case "status":
      if (live) throw new Error("status does not accept --live.");
      await sessionStatus();
      return;
    default:

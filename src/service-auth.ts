...
type BrowserFetchResult =
  ...
  | { kind: "authentication-http-error"; status: number }
  | { kind: "authentication-shape-error" }
  | { kind: "service-http-error"; status: number }
  | { kind: "service-json-error" };
 ...
 */
export async function fetchAuthenticatedDdbJson(page: Page, serviceUrl: string): Promise<unknown> {
  const url = validateServiceUrl(serviceUrl);
  const result = await page.evaluate(async ({ authUrl, timeoutMs, url }): Promise<BrowserFetchResult> => {
    ...
    if (!authResponse.ok) return { kind: "authentication-http-error", status: authResponse.status };
    ...
    const token = authBody && typeof authBody === "object" && !Array.isArray(authBody)
      ...
      : null;
    if (typeof token !== "string" || token.length === 0) return { kind: "authentication-shape-error" };
    ...
    if (!response.ok) return { kind: "service-http-error", status: response.status };
    try {
  ...
  if (
    result.kind === "authentication-shape-error" ||
  ...
  if (result.kind === "service-http-error") throw new DdbServiceRequestError(result.status);

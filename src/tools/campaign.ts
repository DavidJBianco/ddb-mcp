...
interface ExtractedTextSection {
  present: boolean;
  text: string;
}

interface ExtractedCampaignDom {
  name: string;
  currentUserId: string | null;
  dmControlsVisible: boolean;
  description: ExtractedTextSection;
  publicNotes: ExtractedTextSection;
  privateNotes: ExtractedTextSection;
  characterSectionPresent: boolean;
  characters: Array<{ id: string; name: string }>;
  inviteUrl: string | null;
  administrationLinks: Array<{ marker: string; url: string }>;
}
...
function canonicalValues(values: string[] | undefined, label: string): string[] {
  ...
  return values.map((value) => {
    ...
    if (!normalized) throw new Error(`${label} filters cannot contain empty values.`);
    return normalized;
  });
}
...
function numericId(value: string | number, label: string): string {
  ...
  if (!/^\d+$/.test(id)) throw new Error(`Campaign data contained an invalid ${label}.`);
  return id;
}
...
function normalizeRenderedDate(value: string): string {
  const text = value.normalize("NFKC").trim();
  const iso = text.match(/\b(\d{4}-\d{2}-\d{2})\b/)?.[1];
  if (iso) return validateIsoDate(iso, "Campaign creation date");
...
function normalizeDateTime(value: string): string {
  ...
  if (!Number.isFinite(timestamp)) throw new Error("Campaign details contained an invalid creation date.");
  return new Date(timestamp).toISOString();
}

function normalizeRole(value: string): CampaignRole {
  const role = normalizeText(value.replace(/^role:\s*/i, ""));
  if (role.includes("dungeon master") || role === "dm") return "dungeon_master";
  if (role.includes("player")) return "player";
  return "unknown";
}

function normalizePlayerCount(value: string): number {
  const matched = value.match(/\d+/)?.[0];
  if (!matched) throw new Error("Campaign list contained an invalid player count.");
  return Number(matched);
}

function normalizeSharing(value: string): boolean {
  const sharing = normalizeText(value);
  if (/disabled|not\s+enabled|inactive|\boff\b/.test(sharing)) return false;
  if (/enabled|active|\bon\b/.test(sharing)) return true;
  throw new Error("Campaign list contained an unrecognized content-sharing state.");
}

function canonicalCampaignUrl(id: string): string {
  return `${DDB_ORIGIN}/campaigns/${id}`;
}

export function validateCampaignListRequest(request: CampaignListRequest): void {
  ...
  for (const role of request.roles ?? []) campaignRoleSchema.parse(role);
  ...
  for (const [label, value] of [["min_players", request.minPlayers], ["max_players", request.maxPlayers]] as const) {
  ...
  if (request.sortBy !== undefined) campaignSortFieldSchema.parse(request.sortBy);
...
export function normalizeCampaignList(
  extracted: ExtractedCampaignList,
  request: CampaignListRequest = {}
): CampaignListResult {
  validateCampaignListRequest(request);
  if (!extracted.recognized) throw new Error("D&D Beyond campaign-list structure was not recognized.");

  const campaigns: CampaignListResult["campaigns"] = extracted.items.map((item) => {
    const id = numericId(item.id, "campaign ID");
    const name = item.name.normalize("NFKC").trim().replace(/\s+/g, " ");
    if (!name) throw new Error("Campaign list contained an empty campaign name.");
    return {
      id,
      name,
  ...
  for (const campaign of campaigns) {
    if (seen.has(campaign.id)) throw new Error("Campaign list contained a duplicate campaign ID.");
  ...
  const filtered = campaigns.filter((campaign) => {
    if (normalizedNames.length > 0 && !normalizedNames.some((name) => normalizeText(campaign.name).includes(name))) return false;
    if (campaignIds.length > 0 && !campaignIds.includes(campaign.id)) return false;
    if (roles.length > 0 && !roles.includes(campaign.role)) return false;
    if (request.createdOnOrAfter !== undefined && campaign.createdOn < request.createdOnOrAfter) return false;
    if (request.createdOnOrBefore !== undefined && campaign.createdOn > request.createdOnOrBefore) return false;
    if (request.minPlayers !== undefined && campaign.playerCount < request.minPlayers) return false;
    if (request.maxPlayers !== undefined && campaign.playerCount > request.maxPlayers) return false;
    if (request.contentSharingEnabled !== undefined && campaign.contentSharingEnabled !== request.contentSharingEnabled) return false;
    return true;
  });
  ...
  filtered.sort((left, right) => {
    ...
    if (field === "role") primary = left.role.localeCompare(right.role);
    if (field === "created") primary = left.createdOn.localeCompare(right.createdOn);
    if (field === "players") primary = left.playerCount - right.playerCount;
    if (field === "content_sharing") primary = Number(left.contentSharingEnabled) - Number(right.contentSharingEnabled);
...
export async function listMyCampaigns(
  ...
  const extracted = (await cachedMetadata(
    context,
    "campaign-summaries",
    CAMPAIGN_LIST_CACHE_TTL_MS,
    async () => {
      ...
      return page.evaluate((): ExtractedCampaignList => {
...
function isCampaignResponse(response: Response, origin: string, path: string): boolean {
  try {
...
async function waitForOptionalResponse(page: Page, origin: string, path: string): Promise<Response | null> {
  try {
...
async function responseJson(response: Response | null): Promise<unknown | null> {
...
function unavailable() {
  return { state: "unavailable" as const, value: null, provenance: null };
}

function populated<T>(value: T, provenance: "campaign-details-v1" | "active-short-characters" | "rendered-dom" | "derived") {
  const empty = (typeof value === "string" && value.length === 0) || (Array.isArray(value) && value.length === 0);
  return { state: empty ? "empty" as const : "available" as const, value, provenance };
}

function renderedSection(section: ExtractedTextSection) {
  return section.present ? populated(section.text, "rendered-dom") : unavailable();
}

function safeDdbUrl(value: string): URL | null {
  try {
    const url = new URL(value);
    if (url.origin !== DDB_ORIGIN || url.username || url.password || url.protocol !== "https:") return null;
    return url;
  } catch {
    return null;
  }
...
function campaignLinkKind(marker: string): "edit" | "manage" | "settings" | "other" {
...
export async function getCampaign(
  context: BrowserContext,
  campaignId: string,
  request: CampaignDetailRequest = {}
): Promise<CampaignDetailResult> {
  if (!/^\d+$/.test(campaignId)) throw new Error("Campaign ID must contain only decimal digits.");
  ...
  const dom = await page.evaluate(({ includePrivateNotes, includeInviteLink, includeAdministrationLinks }): ExtractedCampaignDom => {
    const textSection = (selector: string): ExtractedTextSection => {
    ...
    const characters = characterCards.flatMap((element) => {
      const link = element.querySelector("a[href*='/characters/']") as HTMLAnchorElement | null;
    ...
    if (includeInviteLink) {
      const invite = document.querySelector(".ddb-campaigns-invite-container [data-clipboard-text], .ddb-campaigns-invite-wrapper [data-clipboard-text], .ddb-campaigns-invite-container a[href]") as HTMLElement | null;
      inviteUrl = invite?.getAttribute("data-clipboard-text") ?? (invite instanceof HTMLAnchorElement ? invite.href : null);
    }
    const administrationLinks: Array<{ marker: string; url: string }> = [];
    if (includeAdministrationLinks) {
      document.querySelectorAll("a[href]").forEach((node) => {
        ...
        if (!link.pathname.startsWith("/campaigns/")) return;
        if (!/edit|manage|setting/.test(marker)) return;
        if (/deactivate|delete|remove|reset|leave/.test(marker)) return;
        if (link.hasAttribute("data-confirm-message")) return;
    ...
    const dmControlsVisible = Boolean(document.querySelector(
      ".ddb-campaigns-detail-body-dm-notes-private, .ddb-campaigns-invite-wrapper, [class*='campaign'][class*='deactivate']"
    ));
    return {
  ...
  if (!name) throw new Error("D&D Beyond campaign page did not expose a campaign name.");

  const detailsDmId = details ? numericId(details.dmId, "dungeon master ID") : null;
  let viewerRole: CampaignRole = "unknown";
  if (detailsDmId && dom.currentUserId) viewerRole = detailsDmId === dom.currentUserId ? "dungeon_master" : "player";
  else if (dom.dmControlsVisible) viewerRole = "dungeon_master";
  ...
  let invite: CampaignDetailResult["campaign"]["links"]["invite"] = unavailable();
  if (includeInviteLink && dom.inviteUrl) {
    const url = safeDdbUrl(dom.inviteUrl);
    if (url && /\/campaigns\/(?:join|invite)(?:\/|$)/.test(url.pathname)) {
      invite = populated({ kind: "invite" as const, url: url.href }, "rendered-dom");
    }
  }
  const administration = includeAdministrationLinks
    ? populated([...new Map(dom.administrationLinks.flatMap((candidate) => {
        ...
        if (!url || !url.pathname.startsWith("/campaigns/") || /deactivate|delete|remove|reset|leave/i.test(url.pathname)) return [];
        const value = { kind: campaignLinkKind(candidate.marker), url: url.href };
        return [[`${value.kind}:${value.url}`, value] as const];
      })).values()], "rendered-dom")
    : unavailable();

  const result = {
    source: "dndbeyond-campaign" as const,
    ...
    campaign: {
      ...
      viewerRole,
      identityProvenance: details ? "campaign-details-v1" as const : "rendered-dom" as const,

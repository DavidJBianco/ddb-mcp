...
function parseJson(text, label) {
  try {
    ...
    throw new Error(`${label} returned malformed JSON; response body suppressed`);
  }
}
...
async function callText(client, name, args = {}) {
  ...
  try {
    ...
    throw new Error(`${name} transport failed; diagnostics suppressed`);
  }

  if (result.isError) throw new Error(`${name} returned a tool error; response body suppressed`);
...
test(
  ...
  async (t) => {
    ...
    try {
      ...
      throw new Error("live MCP initialization failed; diagnostics and paths suppressed");
    }
    ...
    await t.test("restores the external session and lists characters", async () => {
      characters = parseJson(await callText(client, "ddb_list_characters"), "ddb_list_characters");
    ...
    await t.test(
      ...
      async () => {
        ...
        const result = parseJson(
          await callText(client, "ddb_get_character", { character_id: character.id }),
          "ddb_get_character"
        );
    ...
    await t.test(
      ...
      async () => {
        ...
        await callText(client, "ddb_download_character", {
    ...
    await t.test("exercises rendered character fallback with a nonexistent ID", async () => {
      const fallback = parseJson(
        await callText(client, "ddb_get_character", {
    ...
    await t.test("lists campaigns without exposing campaign data", async () => {
      campaigns = parseJson(await callText(client, "ddb_list_campaigns"), "ddb_list_campaigns");
    ...
    await t.test(
      ...
      async () => {
        ...
        const campaign = parseJson(
          await callText(client, "ddb_get_campaign", { campaign_id: campaignId }),
          "ddb_get_campaign"
        );
    ...
    await t.test("navigates safely and reads the current page", async () => {
      const navigated = await callText(client, "ddb_navigate", {
      ...
      const current = await callText(client, "ddb_current_page");
    ...
    await t.test("performs a read-only search", async () => {
      const results = await callText(client, "ddb_search", { query: "shield", category: "spells" });
    ...
    await t.test("lists the owned library without exposing titles", async () => {
      const library = parseJson(await callText(client, "ddb_list_library"), "ddb_list_library");
    ...
    await t.test(
      ...
      async () => {
        ...
        const content = await callText(client, "ddb_read_book", { book_slug: slug });
    ...
    await t.test("uses generic interaction only for a screenshot", async () => {
      const response = await callText(client, "ddb_interact", { action: "screenshot", selector: "body" });
